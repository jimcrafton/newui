...
namespace waavs {
    ...
    // Trim left/right whitespace with chunk_trim()
        ...
        const bool preserveSpace)
    {
        ...
        while (s)
        {
            ...
            ByteSpan token = bspan_read_until(s, wsp);
    ...
    struct SVGFlowRoot final : public SVGGraphicsElement
    {
        ...
        void parseRectFromElement(const XmlElement& elem)
        {
            ...
            while (xmlattribute_read_next(src, k, v))
            {
                InternedKey key = WSNameSet::INTERN(k);
        ...
        void bindSelfToContext(IDrawGraphics* ctx, IAmGroot* groot) override
        {
            ...
            if (fHasFlowRect)
            {
                ...
                const double x = fX.isSet() ? fX.calculatePixels(font, vw, 0, dpi) : 0.0;
                const double y = fY.isSet() ? fY.calculatePixels(font, vh, 0, dpi) : 0.0;
                const double w = fW.isSet() ? fW.calculatePixels(font, vw, 0, dpi) : 0.0;
                const double h = fH.isSet() ? fH.calculatePixels(font, vh, 0, dpi) : 0.0;

                fFlowBox = { x, y, w, h };
            }
        }

        void drawLine(IDrawGraphics* ctx, const ByteSpan& txt, double x, double y)
        {
            uint32_t porder = ctx->state().getPaintOrder();
        ...
        void drawSelf(IDrawGraphics* ctx, IAmGroot* groot) override
        {
            ...
            {
                ByteSpan ta = getAttributeByName(svgattr::text_align());
                bspan_trim(ta, chrWspChars);
                if (ta) {
                    InternedKey tak = WSNameSet::INTERN(ta);

...
namespace waavs
{
    ...
    constexpr auto kDegToRad = 0.017453292519943295;    // PI / 180
    constexpr auto RadToDeg = 57.29577951308232;        // 180 / PI
    constexpr auto Sqrt2 = 1.41421356237309504880;      // sqrt(2.0)
...
namespace waavs
{
    ...
    // Local patch (newui, pinned commit 2bacca9c) - constexpr dropped from
    // this and the other std::<cmath>/std::swap wrappers below: MSVC
    // (error C3615) rejects a constexpr function whose body can never
    // actually be a constant expression, since none of std::swap/isfinite/
    // fmaf/floor/ceil/isnan/atan/atan2 are constexpr pre-C++20/23 - GCC/
    // Clang let this slide since nothing here is ever actually invoked in
    // a constant-expression context. Purely a language-strictness issue,
    // not a behavior change - every call site is runtime anyway.
...
namespace waavs
{
    ...
    // See the int swap() patch note above - same reason for isfinite/lerp/
    // swap here, and floor/ceil/isNaN/atan/atan2 further below.
    static INLINE bool isfinite(float a)  noexcept { return std::isfinite(a); }
    ...
    static INLINE float lerp(float a, float b, float u) noexcept { return fmaf((b - a), u, a); }

    static INLINE  void swap(float& a, float& b)  noexcept { return std::swap(a, b); }
    ...
    static INLINE float floor(float a) noexcept { return std::floor(a); }
    static constexpr INLINE int floorI(float x) noexcept
    {
        int i = (int)x;
        return (x < (float)i) ? (i - 1) : i;
    }
    static INLINE float ceil(float a) noexcept { return std::ceil(a); }
    static INLINE bool isNaN(const float a) noexcept { return std::isnan(a); }
...
namespace waavs
{
    ...
    // distance_to_line()
    // Calculate the distance from a point to a line passing
    // through two points (a and b).
    // General line formula: Ax + By + C = 0, 
    //  where A = ay - by, B = bx - ax, C = ax * by - bx * ay
    static INLINE double distance_to_line(
        double px, double py,
        double ax, double ay,
        double bx, double by) noexcept
    {
        ...
        double num = std::abs(A * px + B * py + C);

...
namespace waavs {
	...
	struct SVGSymbolNode : public SVGGraphicsElement	
	{
		...
		virtual void fixupSelfStyleAttributes(IAmGroot*) override
		{
            ...
            lengthValue_parse(getAttribute(svgattr::refX()), fRefX);
            lengthValue_parse(getAttribute(svgattr::refY()), fRefY);
		...
		void drawSelf(IDrawGraphics* ctx, IAmGroot* groot) override
		{
			...
			const WGRectD instanceVP = ctx->state().getViewport();
			...
			if (!resolveViewState(instanceVP, 
				fDocVP, 
				false, // isTopLevel
				dpi, 
				&fontOpt, 
			...
			ctx->state().setViewport(vp.fViewBox);

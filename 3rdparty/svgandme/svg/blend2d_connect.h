...
namespace waavs
{
    ...
    static INLINE Surface surfaceFromBLImage(const BLImage& img) noexcept
    {
        ...
        Surface surf = Surface::createBorrowed(
    ...
    {
        BLImage img;
        img.create_from_data((int)surf.width(), (int)surf.height(), BL_FORMAT_PRGB32, (uint8_t*)surf.data(), surf.stride());
        return img;
    }
    ...
    struct BLPathProgramExec
    {
        ...
        void execute(uint8_t op, const float* a) noexcept
        {
            ...
            case OP_CUBICTO: {
                ...
                cx = a[4]; cy = a[5];
                hasCP = true;
                path.cubic_to(a[0], a[1], a[2], a[3], a[4], a[5]);
                pathJustClosed = false;
            } break;
            ...
            case OP_ARCTO: {
                ...
                path.elliptic_arc_to(BLPoint(a[0], a[1]), xrot, larc, swp, BLPoint(a[5], a[6]));
                pathJustClosed = false;
            } break;

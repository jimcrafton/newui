...
namespace waavs
{
    ...
    struct SVGFilterPrimitiveElement : public SVGGraphicsElement
    {
        ...
        void fixupSelfStyleAttributes(IAmGroot* groot) override
        {
            ...
            if (getAttribute(filter::in(), fInAttr)) { fIn = WSNameSet::INTERN(fInAttr); }
            if (getAttribute(filter::in2(), fIn2Attr)) { fIn2 = WSNameSet::INTERN(fIn2Attr); }
            if (getAttribute(filter::result(), fResultAttr)) { fResult = WSNameSet::INTERN(fResultAttr); }
            ...
            lengthValue_parse(getAttribute(svgattr::x()), x);
            lengthValue_parse(getAttribute(svgattr::y()), y);
            lengthValue_parse(getAttribute(svgattr::width()), w);
            lengthValue_parse(getAttribute(svgattr::height()), h);

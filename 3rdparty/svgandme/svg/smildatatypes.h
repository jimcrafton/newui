    ...
    */
    ...
    static bool readTimeComponent(ByteSpan& bs, double& outSeconds, AnimMetricType metricType)
    {
        ...
        if (twoDigitsRequired) {
            uint64_t digits{ 0 };
            if (!fixed_digits_read(s, digits, 2))
                return false;
            ...
            uint64_t myint{ 0 };
            size_t digitsRead{ 0 };
            if (!u64_read(s, myint, digitsRead))
                return false;
            value = myint;
        }
    ...
    static bool parseTimecountValue(ByteSpan& part1, double& outSecs)
    {
        ...
        if (!number_read(part1, value)) return false;
    ...
    static bool parseClockDuration(const ByteSpan& bs, double& outSecs)
    {
        ...
        ByteSpan part1 = bspan_read_until(s, ':');
        ByteSpan part2 = bspan_read_until(s, ':');
...
namespace waavs {
    ...
    bool parseTimingSpecifier(const ByteSpan& bs, int& kind)
    {
        ...
        // Check for wallclock
        if (bspan_starts_with(s, "wallclock")) {
        ...
        if (bspan_starts_with(s, "indefinite")) {
...
namespace waavs {
    ...
    */
    ...
    static bool parseEventValue(
        const ByteSpan& inChunk,
        ByteSpan& outBase,
        ByteSpan& outSymbol,
        int& sign,
        double& clockValue) noexcept
    {
        outBase.reset();
        outSymbol.reset();
        sign = 0;
        ...
        ByteSpan span = inChunk;
        bspan_trim_spaces(span);

        if (!span)
            return false;

        // Optional: Eventbase-element "."
        ByteSpan baseCandidate = span;
        ByteSpan afterBase = bspan_read_until(baseCandidate, '.');

        if (afterBase.end() < span.end())
        {
            if (!parseXsdNCName(afterBase, outBase))
                return false;

            span = baseCandidate;
        }

        bspan_ltrim_spaces(span);

        if (!span)
            return false;

        // Event-symbol, possibly followed by + / - offset.
        ByteSpan symPart = bspan_read_until(span, chrWspChars);
        ByteSpan rest = span;

        ByteSpan symbolCandidate = symPart;
        ByteSpan offsetCandidate{};

        ByteSpan plusRest = symPart;
        ByteSpan beforePlus = bspan_read_until(plusRest, '+');

        if (beforePlus.end() < symPart.end())
        {
            symbolCandidate = beforePlus;
            offsetCandidate = plusRest;
            sign = +1;
        }
        else
        {
            ByteSpan minusRest = symPart;
            ByteSpan beforeMinus = bspan_read_until(minusRest, '-');

            if (beforeMinus.end() < symPart.end())
            {
                symbolCandidate = beforeMinus;
                offsetCandidate = minusRest;
                sign = -1;
            }
        }
        ...
        // Optional offset separated by whitespace: symbol S? (+|-) S? clock
        if (sign == 0)
        {
            bspan_ltrim_spaces(rest);

            if (rest && (*rest == '+' || *rest == '-'))
            {
                sign = (*rest == '+') ? +1 : -1;
                ++rest;
                offsetCandidate = rest;
            }
        }

        if (sign != 0)
        {
            bspan_ltrim_spaces(offsetCandidate);

            if (!offsetCandidate)
                return false;

            if (!parseClockDuration(offsetCandidate, clockValue))
                return false;
        }
